...
<script lang="ts">
	...
	const manualTrigger = async (baseUrl: string, regFormId: string, visitFormId: string) => {
		console.log({visitFormId})
		const sParams = new URLSearchParams({
			baseUrl, regFormId, visitFormId
		})
		const fullUrl = `/workflows/run?${sParams.toString()}`;
		return await fetch(fullUrl).then(() => {
			alert('pipeline triggered manually, Please note: This does not mean the pipeline executed successfully')
		}).catch(err => {
			alert(err.message)
		})
	}
