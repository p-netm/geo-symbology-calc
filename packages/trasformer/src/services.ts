...
export class OnaApiService{
    ...
    async fetchSingleForm(formId: string, getFormPath: string = formEndpoint){
        ...
        return fetch(formUrl, {...this.getCommonFetchOptions()}).then(res => {
    ...
    async fetchPaginatedFormSubmissions(formId: string, submissionCount: number, extraQueryObj: Record<string, string> = {}, getSubmissionsPath: string = submittedDataEndpoint){
        ...
        const fullSubmissionsUrl = `${this.baseUrl}/${getSubmissionsPath}/${formId}`;
            ...
            do{
                ...
                fetchSubmissionPromises.push(() => fetch(paginatedSubmissionsUrl, {...this.getCommonFetchOptions()}).then(res => res.json()))
                page = page + 1
            }while(page * pageSize < submissionCount)
            return Promise.all(fetchSubmissionPromises.map(x => x())).then(jsonArrays => {
                const flattened = flatMap(jsonArrays)
                this.logger(createInfoLog(`Fetched ${flattened.length} submissions for form id: ${formId}`))
                return flattened
            }).catch(err => {
    ...
    async editSubmission(formId: string, submissionPayload: Record<string, unknown>, editSubmissionPath: string = editSumbissionEdnpoint){
        ...
        return fetch(fullEditSubmissionUrl, {...this.getCommonFetchOptions(), method: 'POST', body: JSON.stringify(payload)}).then(res => {
            this.logger(createVerboseLog(`Edited submission with prk: ${submissionPayload._id}`))
            res.json()}).catch(err => {
                this.logger(createErrorLog(`Failed to edit submission with primaryKey: ${submissionPayload._id}`))
                throw err
            })
    }
...
export async function upLoadMarkerColor(service: OnaApiService, formId: string, submission: Record<string, string>, colorCode: Color){
    const newSubmission = {
        ...submission,
